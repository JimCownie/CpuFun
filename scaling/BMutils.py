...
def computeEnv(threadsPerCore=1, numCores=0):
    if numCores == 0:
        numCores = cores
    ...
    env += " KMP_HW_SUBSET=" + str(threadsPerCore) +"T KMP_AFFINITY='compact,granularity=fine' "
    return env
...
def runScaling(runDesc, test, threadsPerCore=1, printOnly=False):
    ...
    for numCores in coreList:
        ...
        for opt in runDesc.subOptions.get(test, ("",)):
            for extra in runDesc.extraOptions.get(test, ("",)):
                print (f"Env: {env}, test: {test}, opt: {opt}, extra: {extra}")
                execute(
                    env + runDesc.image + " " + test + opt + " " + extra,
                    outputName(runDesc.outputNamePrefix + test + opt + str(threadsPerCore)+"T" +
                               "_" + extra),
                    printOnly
                )
