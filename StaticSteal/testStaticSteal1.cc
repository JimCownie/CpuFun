...
bool contiguousWork::incrementBase(unsignedType * basep) {
  ...
  setBase(oldBase + 1);
  // We need to ensure that the subsequent load does not float above
  // this, so need a memory fence here to ensure that no loads float
  // above the preceding store.
